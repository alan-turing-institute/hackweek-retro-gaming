...
class SwarmController:
    ...
    def __init__(self, screen_width, offset_y, initial_frame_ticks) -> None:
        ...
        for y in range(7):
            for x in range(10):
                invader: InvaderModel = InvaderModel(x=160 + (x * 48) + 8, y=(y * 32) + offset_y, alien_type=y % 2)
...
class InvaderView:
    ...
    def render(self, surface: Surface):
        for invader in self.swarm.invaders:
            surface.blit(self.image, (invader.x, invader.y, 32, 32), (invader.anim_frame * 32, 32 * invader.alien_type, 32, 32))
